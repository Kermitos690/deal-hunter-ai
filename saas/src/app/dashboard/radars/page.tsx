...
export default async function RadarsPage() {
  ...
  return <div>
    <div className="flex justify-between"><div><h1 className="text-3xl font-black">Mes radars</h1><p className="text-slate-400">Privés • plan {user.plan} • {(data??[]).filter((r:any)=>r.is_active).length}/{limits.activeRadars===Number.MAX_SAFE_INTEGER?"∞":limits.activeRadars} actifs • scan minimum {limits.minScanMinutes} min</p></div><Link className="button" href="/dashboard/radars/new">Créer</Link></div>
