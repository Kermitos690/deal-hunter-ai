...
export default async function DealDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const decisionLabels:Record<string,string>={APPROVED:"VALIDÉ",CONDITIONAL:"SOUS CONDITIONS",REVIEW_REQUIRED:"REVUE REQUISE",REJECTED:"ÉCARTÉ"};
  const evidenceLabels:Record<string,string>={A:"forte",B:"correcte",C:"limitée",D:"faible"};
  const sortedComparables = [...(deal.deal_score_comparables ?? [])].sort((a:any,b:any)=>b.weight-a.weight);
  const soldCount = sortedComparables.filter((c:any)=>c.evidence_type==="SOLD").length;
  const activeCount = sortedComparables.filter((c:any)=>c.evidence_type==="ACTIVE_LISTING").length;
  return <div><div className="flex flex-wrap justify-between gap-4"><div><div className="flex gap-2"><span className="badge">{decisionLabels[deal.decision_status]??"REVUE REQUISE"}</span><span className="badge">Preuve {deal.evidence_grade??"D"}</span></div><h1 className="mt-3 text-3xl font-black">{p.title}</h1><p className="text-slate-400">{p.source} • {p.condition_grade}</p><p className="mt-2 max-w-3xl text-sm text-slate-300">{deal.decision_rationale??"Analyse humaine complémentaire requise."}</p></div><DealActions id={deal.id} /></div>
    ...
    <section className="card mt-6"><h2 className="font-bold">Qualité de la preuve</h2><p className="mt-1 text-sm text-slate-400">La décision est prudente : les annonces actives cadrent le marché, les ventes conclues pèsent davantage.</p><div className="mt-4 grid gap-3 md:grid-cols-4"><Metric k="Niveau" v={`${deal.evidence_grade??"D"} — ${evidenceLabels[deal.evidence_grade]??"faible"}`} /><Metric k="Ventes conclues" v={`${soldCount}`} /><Metric k="Annonces actives" v={`${activeCount}`} /><Metric k="Confiance" v={deal.market_confidence??"LOW"} /></div></section>
    ...
    <section className="card mt-6"><h2 className="font-bold">Comparables utilisés</h2><p className="mt-1 text-sm text-slate-400">{deal.comparable_count} comparable(s) • confiance {deal.market_confidence}</p>
      {sortedComparables.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-slate-500"><tr><th className="pb-3">Preuve</th><th>Source</th><th>Produit</th><th>Prix</th><th>Date</th><th>État</th><th>Fiabilité</th><th>Poids</th></tr></thead><tbody>{sortedComparables.map((c:any)=><tr className="border-t border-white/10" key={c.id}><td className="py-3"><span className="badge">{c.evidence_type==="SOLD"?"Vente conclue":c.evidence_type==="ACTIVE_LISTING"?"Annonce active":"Signal"}</span></td><td>{c.source}</td><td>{c.evidence_url?<a className="text-mint" href={c.evidence_url} target="_blank">{c.title??c.model??"Voir"}</a>:c.title??c.model??"—"}</td><td>{c.price} {c.currency}</td><td>{c.sold_at?new Date(c.sold_at).toLocaleDateString("fr-CH"):"—"}</td><td>{c.condition_grade??"—"}</td><td>{c.confidence}</td><td>{Number(c.weight).toFixed(3)}</td></tr>)}</tbody></table></div>:<div className="mt-4 rounded-xl bg-orange-400/10 p-4 text-orange-100">Aucun comparable solide enregistré. L’estimation est prudente et ne doit pas être considérée comme un prix de revente garanti.</div>}
    </section>
