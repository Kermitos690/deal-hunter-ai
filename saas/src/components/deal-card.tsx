...
export function DealCard({ deal }: { deal: any }) {
  ...
  return <article className="card overflow-hidden p-0"><div className="aspect-video bg-black/30">{image ? <img src={image} alt={product.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-slate-600">Aucune photo</div>}</div>
    <div className="p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex gap-2"><span className="badge">{product.source}</span><span className="badge">{statusLabels[deal.decision_status]??"REVUE REQUISE"} • preuve {deal.evidence_grade??"D"}</span></div><h2 className="mt-2 text-lg font-bold">{product.title}</h2></div><span className={`rounded-xl px-3 py-2 text-lg font-black ${deal.total_score >= 85 ? "bg-green-400/15 text-mint" : "bg-orange-400/15 text-orange-200"}`}>{deal.total_score}</span></div>
